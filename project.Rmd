...
partywise_votes <- partywise_votes%>%
  dplyr::select(-Party, -Votes, -Valid_Votes, -Electors, -Constituency_Type)%>%
  group_by(State_Name, Constituency_No, Constituency_Name, Assembly_No, Year)%>%
...
``` {r subdist, include=FALSE}
e <- extent(subdistricts_shp)
subdistrict <- subdistricts_shp %>%
  dplyr::select(pc11_sd_id, sd_name)
```

```{r shrug1}
for (i in 1992:1994) {
  rasterfile <- raster(paste0("../shrug/li/Harmonized_DN_NTL_", as.character(i), "_calDMSP.tif"))
  rasterfile <- crop(rasterfile, e, snap="out", filename=paste0("../shrug/li/clipped_", as.character(i), "_calDMSP.tif"), overwrite=TRUE)
  subdistrict[paste0("nightlight_", as.character(i))] <- exact_extract(rasterfile, subdistricts_shp, 'mean', progress=FALSE)
  kable(head(subdistrict))
}
```

``` {r shrug2}
for (i in 2014:2017) {
  rasterfile <- raster(paste0("../shrug/li/Harmonized_DN_NTL_", as.character(i), "_simVIIRS.tif"))
  rasterfile <- crop(rasterfile, e, snap="out", filename=paste0("../shrug/li/clipped_", as.character(i), "_simVIIRS.tif"), overwrite=TRUE)
  subdistrict[paste0("nightlight_", as.character(i))] <- exact_extract(rasterfile, subdistricts_shp, 'mean', progress=FALSE)
}
```

``` {r extract}
kable(head(subdistrict, 20))
paste0("nightlight_", as.character(2015))
```
