...
```{partywise_votes_table, echo=FALSE}
...
kable(head(volatility, 10), caption="Volatility in Assembly Elections")%>%
  kable_styling(font_size=12)
write.csv(partywise_votes, "./data/partywise_deltas_constituency.csv", row.names=FALSE)
...
assembly_data_old <- assembly_shp %>%
...
for (i in 1992:2013) {
  ...
  rasterfile <- crop(rasterfile, ext, snap="out", filename=paste0("../shrug/li/clipped_", as.character(i), "_calDMSP.tif"), overwrite=TRUE)
...
for (i in 2014:2021) {
  ...
  rasterfile <- crop(rasterfile, ext_old, snap="out", filename=paste0("../shrug/li/clipped_", as.character(i), "_simVIIRS.tif"), overwrite=TRUE)
...
```{r parties_by_election_table, echo=FALSE}
