...
partywise_deltas <- read.csv("./data/partywise_deltas.csv")
...
ggplot(means, aes(x = cycle)) +
  geom_line(aes(y = incumb_share, colour = "incumb_share")) + 
  geom_line(aes(y = state_incumb_share, colour = "state_incumb_share")) +
  geom_line(aes(y = nat_incumb_share, colour = "nat_incumb_share")) +
...
``` {r nightlights_table, echo=FALSE, eval=FALSE}
...
panel <- panel%>%
  ...
  filter(is.finite(percent_change))%>%
  filter(!is.na(percent_change_3))%>%
  filter(is.finite(percent_change_3))%>%
  filter(!is.na(percent_change_p3))%>%
  filter(is.finite(percent_change_p3))%>%
  filter(!is.na(percent_change_t))%>%
  filter(is.finite(percent_change_t))%>%
...
Q1_1 <- quantile(panel$percent_change, .25)
Q3_1 <- quantile(panel$percent_change, .75)
IQR_1 <- IQR(panel$percent_change)

Q1_2 <- quantile(panel$percent_change_t, .25)
Q3_2 <- quantile(panel$percent_change_t, .75)
IQR_2 <- IQR(panel$percent_change_t)

panel <- panel%>%
  filter((percent_change > (Q1_1 - 1.25*IQR_1)) & (percent_change < (Q3_1 + 1.25*IQR_1)))%>%
  filter((percent_change_t > (Q1_2 - 1.25*IQR_2)) & (percent_change_t < (Q3_2 + 1.25*IQR_2)))%>%
  mutate(state = gsub("_", " ", state))%>%
...
ggplot(panel, aes(x=percent_change)) +
  geom_histogram(color="skyblue", fill="white") +
...
means <- panel%>%
  group_by(delta_year)%>%
  summarise(mean_nightlights=mean(nightlights), percent_change=mean(percent_change), percent_change_t=mean(percent_change_t), across())%>%
  ungroup()%>%
  group_by(cycle)%>%
...
ggplot(means) +
  geom_line(aes(x=delta_year, y=percent_change, colour = "percent_change")) +
  geom_line(aes(x=delta_year, y=percent_change_t, colour = "percent_change_t")) +
...
means <- panel%>%
  group_by(state)%>%
  summarise(mean_nightlights=mean(nightlights), percent_change=mean(percent_change), percent_change_t=mean(percent_change_t), mean_volatility=mean(volatility), across())%>%
  ungroup()

ggplot(means, aes(x=percent_change, y=mean_volatility, colour=state)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "blue") + 
...
means <- panel%>%
  group_by(delta_year)%>%
  summarise(mean_nightlights=mean(nightlights)*500, percent_change=mean(percent_change), percent_change_t=mean(percent_change_t), mean_volatility=mean(volatility), across())%>%
  ungroup()
...
temp_panel <- inner_join(sndp, temp_vol, by=c("state" = "State_Name", "delta_year" = "election_year"))
kable(head(temp_panel, 20))
```

``` {r model_percent_change_temp, eval=TRUE}
model_rc <- plm(volatility ~ pc_sndp + delta_year + factor(state), data = temp_panel, index=c("state", "delta_year"), model="within")

stargazer(model_rc, type="text", report=("vcsp*"), omit=c("state", "delta_year"))
```
...
``` {r model_percent_change_basic, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_t + election_year + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_t + election_year + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_t + election_year + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_t + election_year + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")
...
model_ac2 <- plm(incumb_share ~ absolute_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_ac3 <- plm(state_incumb_share ~ absolute_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_ac4 <- plm(nat_incumb_share ~ absolute_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_ac, model_ac2, model_ac3, model_ac4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r model_percent_change, eval=TRUE}
model_rc <- plm(volatility ~ percent_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("ac_name", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("ac_name", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("ac_name", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("ac_name", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r model_percent_change_3, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r model_percent_change_t, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_p3_t + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_p3_t + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_p3_t + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_p3_t + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r model_percent_change_p3, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_p3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_p3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_p3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_p3 + election_year + delta_ENOP + delta_Turnout + years_since_election + Electors + factor(state), data = panel, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r model_graph, echo=FALSE, eval=TRUE}
ggplot(panel, aes(x = percent_change, y = volatility)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change vs Volatility")

ggplot(panel, aes(x = percent_change, y = incumb_share)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change vs Incumbent Change")

ggplot(panel, aes(x = percent_change, y = state_incumb_share)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change vs State Incumbent Change")

ggplot(panel, aes(x = percent_change, y = nat_incumb_share)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change vs National Incumbent Change")

ggplot(panel, aes(x = percent_change_t, y = volatility)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change over tenure vs Volatility")

ggplot(panel, aes(x = percent_change_t, y = incumb_share)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
  labs(title = "Relative Change over tenure vs Incumbent Change")

ggplot(panel, aes(x = percent_change_t, y = state_incumb_share)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") + 
...
ggplot(filterpanel, aes(x=percent_change)) +
  geom_histogram(color="skyblue", fill="white") +
  labs(title = "Distribution of relative change values") +
  theme(axis.text.x = element_text(angle = 45))
```
...
``` {r filter_model_percent_change, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_p3 + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r filter_model_percent_change_t, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_t + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel, index=c("state", "ac_name", "election_year"), model="within")
...
``` {r filter2_model_percent_change, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_p3 + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_p3 + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

stargazer(model_rc, model_rc2, model_rc3, model_rc4, type="text", report=("vcsp*"), omit=c("state", "election_year"))
```

``` {r filter2_model_percent_change_t, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_t + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc2 <- plm(incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc3 <- plm(state_incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")

model_rc4 <- plm(nat_incumb_share ~ percent_change_t + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")
...
``` {r filter_model_percent_change2, eval=TRUE}
filterpanel2 <- filterpanel2%>%
  filter(percent_change_p3 < 0)
...
``` {r filter_model_percent_change_t2, eval=TRUE}
model_rc <- plm(volatility ~ percent_change_t + election_year + factor(state), data = filterpanel2, index=c("state", "ac_name", "election_year"), model="within")
